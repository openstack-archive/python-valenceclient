...
class ListNodes(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        obj = client.nodes.list_nodes()
        columns = ('uuid', 'name', 'podm_id', 'resource_uri', 'index')
...
class ComposeNode(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        client = self.app.client_manager.valence
        obj = client.nodes.compose_node(req)
        columns = ('uuid', 'name', 'podm_id', 'resource_uri', 'index')
...
class ShowNode(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        self.log.debug("take_action(%s)", parsed_args)
        client = self.app.client_manager.valence
        obj = client.nodes.show_node(parsed_args.id)
...
class DeleteNodes(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for node_id in parsed_args.id:
            try:
                client.nodes.delete_node(node_id)
...
class ManageNode(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        client = self.app.client_manager.valence
        obj = client.nodes.manage_node(req)
        columns = ('uuid', 'name', 'podm_id', 'resource_uri', 'index')
        return (columns, (utils.get_item_properties(obj, columns)))
