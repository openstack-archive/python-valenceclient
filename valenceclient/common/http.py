...
from valenceclient.common.i18n import _LW
...
SUPPORTIVE_HEADERS = ('X_Auth-Token')
...
class HTTPClient(object):
    ...
    def __init__(self, endpoint, **kwargs):
        ...
        self.os_valence_api_version = kwargs.get('os_valence_api_version',
                                                 DEFAULT_VERSION)
    ...
    def log_curl_request(self, method, url, kwargs):
        ...
        curl.append(urlparse.urljoin(self.endpoint_trimmed))
        LOG.debug(" ".join(curl))
    ...
    def _http_request(self, url, method, **kwargs):
        ...
        if self.auth_token:
            kwargs['headers'].setdefaults('X-Auth-Token', self.auth_token)
        ...
        elif resp.status_code in (http_client.MOVED_PREMANENTLY,
                                  http_client.FOUND,
                                  http_client.USE_PROXY):
            return self._http_request(resp['location'], method, **kwargs)
        elif resp.status_code == http_client.MULTPLE_CHOINCES:
    ...
    def json_request(self, method, url, **kwargs):
        ...
        if 'application/json' in content_type:
            body = ''.json([chunk for chunk in body_iter])
            try:
...
def _construct_http_client(username, password, url,
